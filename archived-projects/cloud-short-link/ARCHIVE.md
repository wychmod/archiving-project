...
- 归档日期：2026-06-28(首次导入) / 2026-09-13(全历史重导入)
- 导入分支：`main`(源仓库唯一分支)
- 导入提交(源仓库原始 HEAD)：`63e483306746d7cff85db4fb994e4751a90853f8`(本地工作副本,与已删除的 GitHub 原仓库一致)
- **导入方式**:源仓库 history 中**明文提交了生产/开发凭证**(Aliyun OSS AccessKey Secret、MySQL/Redis 密码、SMS app-code、Nacos 默认密码、公网 Redis IP)。2026-09-13 应用户要求改为**完整提交历史导入**(44 个 commit):先在临时克隆中以 `git filter-repo --replace-text` 对全部历史脱敏(凭证 → `<YOUR_*>` 占位符),再以脱敏后的链作为 subtree 源导入,因此归档仓库 history 中**不含**任何真实凭证。原始未脱敏历史仅存于本地工作副本。
