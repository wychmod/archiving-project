...
- 导入提交：`f7e4a65`(`221102_wychmod_dbRouter` 分支 tip;2026-09-13 脱敏重写后 SHA 变为 `31d42e0`,旧快照源 `a94ccb9` 仍包含在导入历史中)
- 提交日期：2022-11-02
- 提交说明：add: 实现和使用分库分表
- 当前状态：已归档(2026-09-13 已用完整提交历史重导入并脱敏),仅保留源码与教学资料作为历史学习参考
...
### 2026-09-13 完整历史重导入与脱敏

- 初次归档采用 squash 导入,提交链不可回溯;2026-09-13 从本地工作副本 `~/IdeaProjects/Lottery-wychmod` 以 `git subtree add` 重新导入 `221102_wychmod_dbRouter` 分支的 **12 个完整提交**(`bf0d5c0` 项目初始化 → `f7e4a65` 实现和使用分库分表)
- 相比旧快照源 `a94ccb9`,分支 tip 多一个 24 文件的收尾提交(测试 DAO 与 Mapper 补充),为该分支的最终完整状态
- 脱敏:导入前用 `git filter-repo --replace-text` 在副本中将全部历史中的本地 MySQL 口令替换为 `<YOUR_DB_PASSWORD>`(filter-repo 重写导致导入后 SHA 与原始 SHA 不同)
